...
names(super)="Ventes de super (en kt)"
```
...
## Ventes de super (en kt)

```{r}
plot.ts(superts, main="Vente de gazoil")

plot.ts(log(superts), main="log des Ventes de gazoil")

logsuperts = log(superts) # pour supprimmer la différence d'amplitude

 superts_decadd = decompose(superts, type = "additive")
 
 plot(superts_decadd)
 
  superts_decmult = decompose(superts, type = "multiplicative")
 
 plot(superts_decmult)
...
## Vente de super en kt / Autocorrelogramme / Autocorrelogramme partiel
...
supertsdiff = diff(superts)

tsdisplay(supertsdiff)
```

# Décomposition additive de la série 

```{r}
 supertsdiff_decadd = decompose(supertsdiff, type = "additive")
 
 plot(supertsdiff_decadd)
```

# Décomposition multiplicative de la série 

```{r}
 supertsdiff_decmult = decompose(supertsdiff, type = "multiplicative")
 
 plot(supertsdiff_decmult)
```

# ARIMA

```{r}
out<-arima(superts,order=c(3,1,2),seasonal=list(order=c(5,0,2)), method = "ML")
...
```{r}
out2<-arima(superts,order=c(4,2,4),seasonal=list(order=c(4,2,4),period=12), method = "ML")
...
```{r}
out3<-arima(superts,order=c(4,2,4),seasonal=list(order=c(4,2,4),period=NA),method="ML")
#out3 = sarima(superts, 4, 2, 4, P = 4, D = 2, Q = 4, S = 12)
tsdiag(out3, gof.lag=60)
```

```{r}
out4<-arima(superts,order=c(4,1,8),seasonal=list(order=c(4,1,8),period=NA),method="ML")
#out3 = sarima(superts, 4, 2, 4, P = 4, D = 2, Q = 4, S = 12)
