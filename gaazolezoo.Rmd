...
- On peut utiliser le log de la série pour supprimmer cette variation d'amplitude dans la série.
- La série n'est pas stationnaire, la variance n'est pas constante dans le temps.

# Caractéristiques de la série.

Certaines remarques ont été faite à la première observation de la série, cependant il conviendrait de vérifier ces appréhension et de poser certaines hypothèses si nécéssaire de sorte à pouvoir cerner toutes les caractéristiques de la série.
...
En observant le log de la série, la variation d'amplitude n'ont pas totalement disparu, de plus la tendance présente désormais une certaine courbure. En passans au log, le problème d'amplitude n'a pas été résolu de façon satisfaisante, pour la tendance, plus elle est linéaire, mieux nous pouvons l'observer de façon globale.
...
Cette hypothèse peut etre confirmée par l'observation de l'autocorrelogramme de la série.
...
L'autocorrellogramme ne décroit pas de façon exponentielle vers 0, par contre on observe des grands piques aux lags 1, 12, 24. ce qui pourrait représenter la périodicité de la série.
...
La section précédente nous a permis de conclure sur une série non-stationnaire, saisonnière de saisonnalité 12. Le choix logique de modèle pour l'estimation de la série est un modèle Seasonnal ARIMA (p,d,q)(P,D,Q) (SARIMA). Les coefficients de ce modèles sont eux aussi fournis par les caractéristiques de la série. Le choix de ces coefficients représentent la principale difficulté, cependant nous pouvons observer différentes transformations de la série afin d'orienter nos différents choix. Tout d'abord explicitons les différentes significations de ces coefficients :
...
L'autocorrelograme de la série décroit de façon exponnentielle, avec un pique au lag 12 qui représente la saisonnalité aussi observé dans l'autocorrelogramme partiel.
...
Pour choisir les paramètres p,d,q qui représentent la modélisation de la partie non saisonnière, on observe l'ACF (autocorrélogramme) et le PACF (autocorrélogramme partiel) à partir du lag 1. L'appréciation de l'ACF permet le choix de q. On choisit q de sorte à pouvoir tenir compte des autocorrélations succéssives qui sortent de la zone d'acceptation (les lignes discontinuent bleues sur la figure).  
Par observation de l'ACF (ci-dessous), les trois premières autocorrélations sont très significatives, surtout la troisième, donc un canditat est $q=3$ .
...
Des quandidats possibles à p sont $p=3$ , $p=5$ .

```{r}
ggPacf(diff(diff(gazolets,12),12))
```

Le paramètre d représente le dégré de différentiation de la partie non saisonnière, vu que la série n'a été différentiée deux fois au lag 12, c'est à dire que sur la période, on suppose que la partie non saisonnière n'a pas été différentiée

### Choix des paramètres P,D,Q

Les paramètres P,Q se choisissent sur les mêmes critères que p,d,q mais appliqués à la partie saisonnière. Pour le choix de ces paramètres, on observe l'ACF et le PACF mais à partir du lag de la période, c'est à dire 12. Donc les parmètres seront choisi afin de tenir compte des piques significatifs à partir du lag 12, contrairement à la partie non saisonnière qui commence au lag 1. Donc $d=0$ .
Le paramètre D représente le dégré de différentiation de la partie saisonnière c'est à dire $D=2$.
