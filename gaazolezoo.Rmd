...
```{r}
out<-arima(gazoletsdiff_decmult$x,order=c(6,0,8))
tsdiag(out, gof.lag=60)
```

# modélisation directe de la série...sans élimination

```{r}
out2<-out<-arima(gazolets,order=c(4,2,4),seasonal=list(order=c(6,2,2),period=12))
tsdiag(out, gof.lag=60)
